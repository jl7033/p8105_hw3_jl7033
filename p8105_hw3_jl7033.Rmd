...
#### Plot of Day of Week/Month/Year on Median Ride Duration

```{r}

citi_df %>%
  separate_wider_delim(time, names = c("month", "year"), delim = "_") %>%
  mutate(weekday_fac = factor(
    weekdays,
    levels = c("Monday", "Tuesday", "Wednesday", "Thursday",
               "Friday", "Saturday", "Sunday"))) %>%
  ggplot(aes(y = duration, x = weekday_fac, fill = weekday_fac)) + 
  geom_boxplot() + 
  scale_y_continuous(limits = c(0, 100)) +
  facet_wrap(month ~ year) + 
  labs(
    x = "Day of the Week",
    y = "Duration of Citi Bike Ride",
    title = "Duration of Citi Bike Rides by Month, Year, and Day of the Week",
...
I decided to use side-by-side boxplots to display the median duration, since the "stripe" in the middle of a boxplot represents the median of the distribution. I reordered the `weekday` variable into a factor in order to show the distributions from Monday through Sunday in the correct order. 

From this plot, we can see that the median ride times were generally longest in July of 2020, and were often longer on the weekends than on the weekdays (though this effect is less noticeable in January 2024). The distributions of ride times were right-skewed for all years, months, and days of the week, with third quartiles mostly between 20 and 25 and maximums well above 100 (outliers above 100 minutes are omitted from the graph for the sake of clarity). Finally, the IQRs were wider in July than they are in January, and were widest in July of 2020.

#### Plot of Month, Membership Status, and Bike Type on Ride Duration
...
citi_df %>%
  ...
  labs(
    x = "Bike Type",
    y = "Duration of Citi Bike Ride",
    title = "Duration of Citi Bike Rides by Month, Membership Status, and Bike Type",
    fill = "Bike Type"
  )
  
```

I once again chose side-by-side boxplots for this problem, as I felt they would more clearly show the distributions of duration than histograms, even with the `position = "dodge"` parameter on. 

Generally, for casual riders, the median duration for classic bikes was higher than the median duration for electric bikes, but for Citi Bike members, the median duration for electric bikes was slightly higher. The IQR of the duration of classic bike rides was higher than the IQR of the duration of electric bike rides for casual riders, but not for Citi Bike members. Similarly to the previous plot, we can see that the distribution of ride durations was right-skewed, regardless of membership status, month, or bike type.